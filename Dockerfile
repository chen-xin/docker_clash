...
ADD ./data/clash-linux /clash
ADD ./data/entrypoint.sh /clash/entrypoint.sh
...
RUN curl -o /clash/Country.mmdb https://github.com/Dreamacro/maxmind-geoip/releases/latest/download/Country.mmdb \
    && CLASH_VERSION=${CLASH_VERSION:-$(curl https://github.com/Dreamacro/clash/tags | grep -o '/Dreamacro/clash/releases/tag/\(v[0-9\.]\+\)' | grep -o 'v[0-9\.]\+' | sort -u | tail -n 1)}\
